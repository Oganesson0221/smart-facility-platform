...
failures=0
json_python=".app-venv/bin/python"
if [[ ! -x "$json_python" ]]; then
  json_python="$(command -v python3 2>/dev/null || true)"
fi
if [[ -z "$json_python" ]]; then
  echo "No Python interpreter found for JSON health checks." >&2
  exit 1
fi
...
check_model() {
  ...
  if printf '%s' "$response" | "$json_python" -c \
...
check_nemo_fire_exit_validation() {
  local base_url
  base_url="$(openai_base_url "${NEMO_AGENT_BASE_URL:-http://127.0.0.1:8010/v1}")"
  local url="${base_url}/chat/completions"
  local image_path="$(pwd)/tests/car4.jpeg"
  ...
  if ! response="$(curl "${curl_args[@]}" -d "$payload" "$url")"; then
    fail "NeMo fire-exit validation: ${url}"
    return
  fi
