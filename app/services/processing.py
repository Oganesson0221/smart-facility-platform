...
def detect_scene_objects(camera: Camera, image: np.ndarray) -> tuple[str, list[Detection]]:
    ...
    return detector.name, detections
...
def _crop_validation_region(
    ...
    x1, y1, x2, y2 = box
    margin = max(24, int(max(x2 - x1, y2 - y1) * 0.12))
...
async def _confirm_fire_exit_candidate(
    ...
    crop = _crop_validation_region(image, polygon_points, obstruction.detection.box)
    ...
    try:
        validation = await validate_fire_exit_obstruction(crop_bytes)
...
async def analyse_image(
    ...
    _, obstructions = _evaluate_obstructions(
        camera, _filtered_detections(camera, image), polygon_points
    )
    incidents = []
    now = datetime.now(timezone.utc)
    for obstruction in obstructions:
        ...
        confirmed, validation, crop_relative = await _confirm_fire_exit_candidate(
            None, image, obstruction, polygon_points
        )
        if not confirmed:
            continue
    ...
    return {
        ...
        "detections": [_obstruction_to_dict(item) for item in obstructions],
...
async def process_video_job(job_id: str, path: Path, polygon_points: list[list[float]]):
    ...
    try:
        ...
        while True:
            ...
            _, evaluated = _evaluate_obstructions(camera, detections, polygon_points)
            currently_blocking = {
